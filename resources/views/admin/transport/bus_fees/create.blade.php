 	...
 	<div class="row mb-4">
 		<div class="col-md-12">
 			<div class="card">
 				...
 				<div class="card-body">
 					<form action="{{route('bus_fee_store')}}" method="post" autocomplete="off">
	 				@csrf
	 					<div class="row">	 						
	 						<div class="col-md-3 form-group">
	 							<label class="required">From <span class="text-muted"><i>(In Kilometer)</i></span></label>
	 							<input type="text" name="bus_fee_from" class="form-control" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" id="from_km">
	 							@error('bus_fee_from')
	 						...
	 						<div class="col-md-3 form-group">
	 							...
	 							<input type="text" name="bus_fee_to" class="form-control" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');" id="to_km">
	 							@error('bus_fee_to')
									<span class="text-danger">
										<strong>{{$message}}</strong>
									</span>
	 						...
	 						<div class="col-md-6 form-group">
	 							...
	 							<input type="text" name="bus_fee_title" class="form-control" readonly="readonly" id="title">
	 							@error('bus_fee_title')
									<span class="text-danger">
										<strong>{{$message}}</strong>
									</span>
	 					...
	 					<div class="row">
	 						<div class="col-md-6 form-group">
	 							...
	 							<input type="text" name="bus_fee_amount" class="form-control" placeholder="Enter amount here" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1');">
	 						...
	 						<div class="col-md-6 form-group">
	 							...
	 							<select class="form-control" name="bus_fee_status">
	 								...
	 								@foreach(STATUS as $key => $status)
	 									<option value="{{$key}}" {{$key=='A' ? 'selected' :''}}>{{$status}}</option>
	 								@endforeach
	 							</select>
